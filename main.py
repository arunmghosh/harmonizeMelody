...
def get_low_pitch(mel):
    # mel is the melody represented by midi notes
    minim = mel[0]
...
def test_output():
    key = Key.key_from_user("G")
    melody = [67, 74, 64, 72, 67, 71, 64, 72]
    symbols = ["I", "V", "vi", "ii", "I", "vi", "ii", "viio"]
    invs = [0, 1, 0, 2, 2, 0, 1, 1]
    chords = numpy.empty(8, dtype=Chord)
    alto = numpy.empty(8, dtype=int)  # these will all contain midi notes
    tenor = numpy.empty(8, dtype=int)
    bass = numpy.empty(8, dtype=int)

    harmony_octave = (get_low_pitch(melody) - (get_low_pitch(melody) % 12)) - 12
    soprano = stream.Stream()
    harmonized = stream.Stream()

    for i in range(len(melody)):
        chords.put(i, Chord.chord_from_symbol(symbols[i], invs[i]))
        alto.put(i, key.degree_to_midi(chords[i].get_alto()) + harmony_octave)
        tenor.put(i, key.degree_to_midi(chords[i].get_tenor()) + harmony_octave)
        bass.put(i, key.degree_to_midi(chords[i].get_bass_note()) + harmony_octave)
        soprano.append(note.Note(melody[i], duration=duration.Duration(type="half")))
        with_harmony = chord.Chord([note.Note(melody[i], duration=duration.Duration(type="half")),
                                    note.Note(alto[i], duration=duration.Duration(type="half")),
                                    note.Note(tenor[i], duration=duration.Duration(type="half")),
                                   note.Note(bass[i], duration=duration.Duration(type="half"))])
        harmonized.append(with_harmony)
        # soprano.write("midi", "test.mid")
        harmonized.write("midi", "test_h.mid")


def display_recs(k, mel, progs):
    cnt = 0
    for p in progs:
        alto = numpy.empty(8, dtype=int)  # these will all contain midi notes
        tenor = numpy.empty(8, dtype=int)
        ...
        harmony_octave = (get_low_pitch(mel) - (get_low_pitch(mel) % 12)) - 12
        soprano = stream.Stream()
        harmonized = stream.Stream()

        for i in range(len(mel)):
            alto.put(i, k.degree_to_midi(p[i].get_alto()) + harmony_octave)
            tenor.put(i, k.degree_to_midi(p[i].get_tenor()) + harmony_octave)
            bass.put(i, k.degree_to_midi(p[i].get_bass_note()) + harmony_octave)
            soprano.append(note.Note(mel[i], duration=duration.Duration(type="half")))
            with_harmony = chord.Chord([note.Note(mel[i], duration=duration.Duration(type="half")),
                                        note.Note(alto[i], duration=duration.Duration(type="half")),
                                        note.Note(tenor[i], duration=duration.Duration(type="half")),
                                        note.Note(bass[i], duration=duration.Duration(type="half"))])
            harmonized.append(with_harmony)
        # soprano.write("midi", "test.mid")
        harmonized.write("midi", "rec" + str(cnt) + ".mid")
        cnt += 1
...
def main():
    ...
    grader = Grader(get_progressions(degrees, palette))
    grader.grade()
    to_user = grader.rank()
    print(len(to_user))
    ...
    for i in range(0, 2000, 500):
        recs.append(to_user[i])
    display_recs(k, melody, recs)
    print("Done!")
